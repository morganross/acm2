...
### Step 4.5: Configure SSL Certificates (REQUIRED)

Windows Python installations don't include CA certificates by default. FPF uses `urllib` which requires system certificates for HTTPS calls to LLM providers.

**Set SSL_CERT_FILE environment variable** (run as Administrator):

```powershell
# Get certifi's certificate bundle path
$certPath = python -c "import certifi; print(certifi.where())"

# Set system-wide environment variable (persists across reboots)
[System.Environment]::SetEnvironmentVariable("SSL_CERT_FILE", $certPath, "Machine")

# Apply to current session
$env:SSL_CERT_FILE = $certPath

# Verify
python -c "import urllib.request; urllib.request.urlopen('https://api.openai.com'); print('SSL OK')"
```

**Expected output**: `SSL OK`

If you skip this step, you will get `ssl.SSLCertVerificationError: certificate verify failed` when calling Google, OpenAI, or other HTTPS APIs.
